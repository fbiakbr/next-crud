...
export default function UpdateUserForm() {
    ...
    const handleSubmit = (e) => {
        ...
        if(Object.keys(formData).length === 0) {
            return <Bug message="Data is Empty" />
        }
    ...
    return (
        <form className="grid lg:grid-cols-2 w4/6 gap-4" onSubmit={handleSubmit}>
            ...
            <button className="flex justify-center text-md w-2/6 bg-indigo-500 text-white px-4 py-2 border rounded-md hover:bg-gray-50 hover:text-indigo-500 hover:border-indigo-500">Update</button>
        </form>
    );
}
